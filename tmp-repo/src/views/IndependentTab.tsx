...
import { Eye, Star, X, Filter, Users, ShieldCheck, CheckCircle2, Activity, Fingerprint } from "lucide-react";
...
export default function IndependentTab() {
  ...
  async function fetchIndependents() {
      try {
        ...
        if (response.ok) {
          ...
          const orgs = (data.organizations || []).filter((o: any) => o.type === "independent");
          setIndependents(orgs);
          if (data.candidates) setCandidates(data.candidates);
        } else {
          const q = query(collection(db, "organizations"), where("type", "==", "independent"));
          const snap = await getDocs(q);
          const independentsData = snap.docs.map(d => ({ id: d.id, ...d.data() }));
         ...
         try {
            ...
            handleFirestoreError(fErr, OperationType.LIST, "independents_governance");
         }
      } finally {
        setLoading(false);
      }
  ...
  const handleDeleteIndependent = async (indId: string) => {
    if (!indId) return;
    if (!window.confirm("Purge this independent specialist node from the governance network?")) return;
    try {
  ...
  const getCandidateCount = (indId: string) => candidates.filter(c => c.vendorId === indId).length;

  return (
    <div className="flex h-full overflow-hidden relative bg-slate-50">
      <div className={`flex-1 p-8 overflow-y-auto transition-all duration-300 ${selectedIndependent ? 'mr-96' : ''}`}>
        <div className="max-w-7xl mx-auto">
          <div className="mb-10 flex justify-between items-end">
            <div>
              <h1 className="text-3xl font-black text-slate-900 tracking-tighter italic uppercase">Independent Specialists</h1>
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Freelancers, solo contractors, and specialist execution nodes.</p>
            </div>
            <div className="flex gap-4">
              <div className="text-right">
                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Global Supply</div>
                <div className="text-xl font-black text-slate-900">{independents.length} Nodes</div>
              </div>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Synchronizing Specialist Layer...</div>
            </div>
          ) : (
              ...
              <table className="w-full text-left text-sm border-collapse">
                ...
                <tbody className="divide-y divide-slate-50">
                  {independents.map(ind => {
                    ...
                    const trustScore = ind.trustScore || Math.min(100, Math.max(70, Math.round(80 + (candCount * 2.5) + (Math.random() * 8))));

                    return (
                      <tr key={ind.id} className={cn("hover:bg-slate-50 transition-all group", selectedIndependent?.id === ind.id && "bg-indigo-50/20")}>
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="font-black text-slate-900 uppercase tracking-tight text-base">{ind.companyName || "Specialist Node"}</span>
                        ...
                        <td className="py-8 text-center">
                          <div className="flex flex-col items-center">
                            <span className="bg-emerald-600 text-white px-4 py-1.5 rounded-xl text-xs font-black shadow-lg shadow-emerald-100 border border-emerald-500">
                        ...
                        <td className="py-8">
                          <div className="flex flex-wrap gap-2">
                            {[
                              ...
                              { label: 'Agreement', active: ind.ndaUploaded || ind.onboardingCompleted },
                  ...
                  {independents.length === 0 && (
                    <tr>
                      <td colSpan={5} className="py-12 text-center text-slate-400 text-sm italic">
                        No active independent specialist matching global HQ routing tags.
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        <div className="absolute right-0 top-0 bottom-0 w-96 bg-white border-l border-slate-200 p-6 flex flex-col shadow-2xl animate-in slide-in-from-right duration-300 z-30">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-black text-slate-900 leading-none">{selectedIndependent.companyName || "Specialist Node"}</h3>
                <p className="text-[10px] text-slate-400 font-mono mt-1 uppercase tracking-widest">Specialist Node Insights</p>
              </div>
            </div>
            <Button variant="ghost" size="icon" onClick={() => setSelectedIndependent(null)} className="h-8 w-8 rounded-full">
              <X size={18} />
            </Button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-6 pr-2">
            <div>
               ...
               <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                     ...
                     <p className="text-xl font-black text-slate-900 mt-1">{getCandidateCount(selectedIndependent.id)}</p>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
            ...
            <div>
              ...
              <div className="space-y-3">
                {candidates.filter(c => c.vendorId === selectedIndependent.id).length === 0 ? (
                  ...
                  candidates.filter(c => c.vendorId === selectedIndependent.id).map(cand => (
                    <div 
                      key={cand.id} 
            ...
            <div className="pt-4 border-t border-slate-100">
               <div className="bg-slate-900 rounded-2xl p-4 text-white">
                  ...
                  <p className="text-[11px] text-slate-400 leading-relaxed italic">
                    "Compliance and KYC active. This specialist node is authorized for direct matching engagement and immediate invoice processing flow."
                  </p>
               </div>
            </div>
          </div>
