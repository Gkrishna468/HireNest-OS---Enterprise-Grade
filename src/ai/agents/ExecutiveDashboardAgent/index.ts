...
import { ExecutiveDashboardAgentMetadata } from './metadata.js';
import { EXECUTIVE_SYSTEM_PROMPT } from './prompts.js';
...
export class ExecutiveDashboardAgent implements HireNestAgent {
  metadata: AgentMetadata = ExecutiveDashboardAgentMetadata;
  ...
  async execute(prompt: string, context: AgentExecutionContext): Promise<AgentResult> {
    ...
    try {
      ...
      let fullPrompt = `${EXECUTIVE_SYSTEM_PROMPT}\n`;
      ...
      const gatewayResponse = await AIGateway.processChat({
        prompt: fullPrompt,
        feature: 'executive_summary', // Mapped capability
      });
...
globalAgentRegistry.register(new ExecutiveDashboardAgent());
