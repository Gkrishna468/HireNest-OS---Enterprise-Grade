        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm sm:items-center items-end">
            <div className="bg-slate-900 w-full max-w-2xl sm:rounded-[24px] rounded-t-[24px] rounded-b-none shadow-2xl border border-slate-700 overflow-hidden flex flex-col max-h-[95vh] sm:max-h-[90vh]">
                ...
                <div className="px-6 py-5 flex justify-between items-center border-b border-slate-800 shrink-0">
                    <div>
                        <h2 className="text-xl font-bold text-white tracking-tight">Add Candidate Profile</h2>
                        <p className="text-[10px] text-indigo-400 font-black uppercase tracking-widest mt-1">POST: {reqTitle} ({reqId})</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 scrollbar-hide text-slate-300 space-y-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <input value={phone} onChange={e => setPhone(e.target.value)} type="text" className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors" placeholder="E.g. +91 98765 43210" />
                        </div>
                            <div>
                            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Total Experience</label>
                            <input value={experience} onChange={e => setExperience(e.target.value)} type="text" className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors" placeholder="E.g. 5 Years" />
                        </div>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Current Location</label>
                            <input value={currentLocation} onChange={e => setCurrentLocation(e.target.value)} type="text" className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors" placeholder="Search or select country, city, town" />
                        </div>
                            <div>
                            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Preferred Location</label>
                            <input value={preferredLocation} onChange={e => setPreferredLocation(e.target.value)} type="text" className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors" placeholder="Search or select country, city, town" />
                        </div>
                    </div>

                    <div>
                        <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Key Skills</label>
                        <input value={keySkills} onChange={e => setKeySkills(e.target.value)} type="text" className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors" placeholder="E.g. React, TypeScript, Redux Toolkit" />
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Notice Period / Availability</label>
                            <input value={noticePeriod} onChange={e => setNoticePeriod(e.target.value)} type="text" className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors" placeholder="E.g. Immediate / 30 D..." />
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Current CTC</label>
                            <input value={currentCtc} onChange={e => setCurrentCtc(e.target.value)} type="text" className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors" placeholder="E.g. 10 LPA" />
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Expected CTC</label>
                            <input value={expectedCtc} onChange={e => setExpectedCtc(e.target.value)} type="text" className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors" placeholder="E.g. 15 LPA" />
                        </div>
                    </div>

                    {/* Resume Upload Box */}
                    <div className="pt-2">
                        <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Upload a Resume</label>
                        ...
                        {!parsed && !isParsing && (
                            ...
                            >
                                ...
                                <p className="text-sm font-semibold text-slate-300 mb-1">Drag & drop or Click to attach Resume document</p>
                                <p className="text-[11px] text-slate-500 font-medium">Supports PDF, DOC, DOCX, TXT. Parses metadata.</p>
                            </div>
                        )}
                        ...
                        {isParsing && (
                            <div className="border border-indigo-500/30 rounded-2xl p-8 text-center bg-indigo-500/5">
                                ...
                                <p className="text-sm font-bold text-indigo-300 uppercase tracking-widest mb-3">AI Extracting Metadata...</p>
                        ...
                        {parsed && aiAnalysis && (
                            <div className="border border-emerald-500/30 rounded-2xl p-6 bg-emerald-500/5 flex flex-col sm:flex-row sm:items-center gap-4">
                                ...
                                <div className="flex-1 text-sm">
                                    <h4 className="font-bold text-emerald-300 mb-1">Resume Attached & AI Parsed</h4>
                                    <p className="text-slate-400 text-xs">Metadata has been intelligently extracted to fill the fields above.</p>
                                </div>
                                <Button variant="outline" className="border-slate-700 text-slate-300 text-xs h-8 px-4" onClick={triggerFileInput}>
                                    Replace File
                                </Button>
                            </div>
                        )}
                ...
                <div className="px-6 py-5 border-t border-slate-800 bg-slate-900 shrink-0 flex justify-end gap-3 rounded-b-[24px]">
                    <Button variant="outline" className="border-slate-700 text-slate-300 hover:bg-slate-800 hover:text-white rounded-xl px-6" onClick={onClose} disabled={isSubmitting}>Cancel</Button>
                    <Button onClick={handleSubmit} className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-medium px-6 shadow-lg shadow-indigo-500/20" disabled={isSubmitting || !name || !email}>
