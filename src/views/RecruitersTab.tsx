...
export default function RecruitersTab() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const orgs = (data.organizations || []).filter(
          (o: any) => o.type === "recruiter",
        );
        setRecruiters(orgs);
        if (data.candidates) setCandidates(data.candidates); // sometimes stored as candidatePool vs candidates in api depending on how it's named in governance-data
        ...
        const recruitersData = snap.docs.map((d) => ({
          id: d.id,
          ...d.data(),
        }));
  ...
  const handleDeleteRecruiter = async (recId: string) => {
    if (!recId) return;
    if (
      !window.confirm("Purge this recruiter node from the governance network?")
    )
      return;
    try {
  ...
  const getCandidateCount = (recId: string) =>
    candidates.filter((c) => c.vendorId === recId).length;

  return (
    <div className="flex h-full overflow-hidden relative bg-slate-50">
      <div
        className={`flex-1 p-8 overflow-y-auto transition-all duration-300 ${selectedRecruiter ? "mr-96" : ""}`}
      >
        <div className="max-w-7xl mx-auto">
          <div className="mb-10 flex justify-between items-end">
            <div>
              <h1 className="text-3xl font-black text-slate-900 tracking-tighter italic uppercase">
                Partner Recruiters Hub
              </h1>
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">
                Independent recruiters & premium sourcing agencies
                orchestration.
              </p>
            </div>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest">
                Synchronizing Recruiter Sourcing Layer...
              </div>
            </div>
          ) : (
                  ...
                  {recruiters.map((r) => {
                    ...
                    const trustScore =
                      ...
                      Math.min(
                        ...
                        Math.max(
                          70,
                          Math.round(75 + candCount * 2.5 + Math.random() * 8),
                        ),
                      );
                    ...
                    return (
                      <tr
                        ...
                        className={cn(
                          ...
                          selectedRecruiter?.id === r.id && "bg-indigo-50/20",
                        )}
                      >
                        ...
                        <td className="py-8 text-center">
                          <span className="text-sm font-black text-amber-600">
                            {
                              dealRooms.filter((dr) => dr.vendorId === r.id)
                                .length
                            }
                          </span>
                        </td>
                          ...
                          <span className="text-sm font-black text-emerald-600">
                            {
                              dealRooms.filter(
                                (dr) =>
                                  dr.vendorId === r.id && dr.stage === "hired",
                              ).length
                            }
                          </span>
                        </td>
                        ...
                        <td className="py-8">
                          <div className="flex flex-wrap gap-2">
                            {[
                              ...
                              {
                                label: "Agreement",
                                active: r.ndaUploaded || r.onboardingCompleted,
                              },
          ...
          <div className="flex-1 overflow-y-auto space-y-6 pr-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  ...
                  <p className="text-xl font-black text-slate-900 mt-1">
                    {getCandidateCount(selectedRecruiter.id)}
                  </p>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-tight">
            ...
            <div>
              ...
              <div className="space-y-3">
                {candidates.filter((c) => c.vendorId === selectedRecruiter.id)
                  .length === 0 ? (
                  ...
                  candidates
                    .filter((c) => c.vendorId === selectedRecruiter.id)
                    .map((cand) => (
                      <div
                        key={cand.id}
            ...
            <div className="pt-4 border-t border-slate-100">
              <div className="bg-slate-900 rounded-2xl p-4 text-white">
                ...
                <p className="text-[11px] text-slate-400 leading-relaxed italic">
                  "Compliance and KYC active. This recruiter is permitted to
                  supply candidates to premium multi-tenant enterprise reqs."
                </p>
              </div>
            </div>
          </div>
