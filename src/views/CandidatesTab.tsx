            ...
            <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
               <div className="p-8 pb-4 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white/95 backdrop-blur z-10">
                  ...
                  <div className="flex items-center gap-2">
                     {isAdmin ? (
                        <Button variant="outline" onClick={() => handleDeleteCandidate(selectedCandidate.originalId || selectedCandidate.id)} className="text-red-500 hover:text-red-600 hover:bg-red-50 font-bold border-red-200">Delete Candidate</Button>
                     ) : (
                        <Button variant="outline" onClick={async () => {
                           await publishEvent({
                              type: "urgent",
                              title: "Delete Request",
                              message: `Vendor requested deletion of ${selectedCandidate.fullName || selectedCandidate.name}`,
                              recipients: ["GLOBAL_ADMIN"]
                           });
                           alert("Delete request sent to Administrator for approval.");
                        }} className="text-slate-500 hover:text-slate-900 border-slate-200">Request Delete</Button>
                     )}
                     ...
                     {selectedCandidate.pipelineStage && selectedCandidate.pipelineStage !== 'ADDED' && (
                        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                           ...
                           <div className="space-y-4 text-sm">
                              {(() => {
                                 const matchedJob = jobs.find(j => j.title === selectedCandidate.reqTitle);
                                 if (!matchedJob) {
                                     return (
                                        <p className="text-slate-500 italic">No detailed requirement data found for target target.</p>
                                     );
                                 }
                                 return (
                                    <>
                                       <div className="flex justify-between items-start">
                                          <div className="font-bold text-slate-900 border-b border-indigo-100 pb-2 mb-2 w-full">{matchedJob.id}<br/><span className="text-indigo-600 inline-block mt-0.5">{matchedJob.title}</span></div>
                                       </div>
                                       <div className="grid grid-cols-2 gap-3 text-xs">
                                          <div><span className="text-slate-500">Client:</span> <span className="font-medium text-slate-900">{matchedJob.company}</span></div>
                                          <div><span className="text-slate-500">Location:</span> <span className="font-medium text-slate-900">{matchedJob.location}</span></div>
                                          <div><span className="text-slate-500">Type:</span> <span className="font-medium text-slate-900">{matchedJob.type}</span></div>
                                          <div><span className="text-slate-500">Budget:</span> <span className="font-medium text-slate-900">{matchedJob.salary}</span></div>
                                       </div>
                                       {(matchedJob.skills && matchedJob.skills.length > 0) && (
                                          <div className="mt-3 pt-3 border-t border-slate-100">
                                             <div className="font-bold text-[10px] uppercase tracking-wider text-slate-500 mb-2">Required Core Skills</div>
                                             <div className="flex flex-wrap gap-1.5">
                                                {matchedJob.skills.map(s => (
                                                   <Badge key={s} variant="outline" className="text-[10px] bg-emerald-50 text-emerald-700 border-emerald-200">{s}</Badge>
                                                ))}
                                             </div>
                                          </div>
                                       )}
                                       {selectedCandidate.aiAnalysis && selectedCandidate.aiAnalysis.breakdown && (
                                          <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-2 lg:grid-cols-4 gap-2 text-center text-xs">
                                              <div className="bg-slate-50 p-2 rounded border border-slate-100">
                                                 <div className="text-[10px] uppercase text-slate-400 font-bold tracking-widest">Skills</div>
                                                 <div className="font-mono text-indigo-600 font-black">{selectedCandidate.aiAnalysis.breakdown.skillsScore}%</div>
                                              </div>
                                              <div className="bg-slate-50 p-2 rounded border border-slate-100">
                                                 <div className="text-[10px] uppercase text-slate-400 font-bold tracking-widest">Experience</div>
                                                 <div className="font-mono text-indigo-600 font-black">{selectedCandidate.aiAnalysis.breakdown.experienceScore}%</div>
                                              </div>
                                              <div className="bg-slate-50 p-2 rounded border border-slate-100">
                                                 <div className="text-[10px] uppercase text-slate-400 font-bold tracking-widest">Domain</div>
                                                 <div className="font-mono text-indigo-600 font-black">{selectedCandidate.aiAnalysis.breakdown.domainScore}%</div>
                                              </div>
                                              <div className="bg-slate-50 p-2 rounded border border-slate-100">
                                                 <div className="text-[10px] uppercase text-slate-400 font-bold tracking-widest">Location</div>
                                                 <div className="font-mono text-indigo-600 font-black">{selectedCandidate.aiAnalysis.breakdown.locationScore}%</div>
                                              </div>
                                          </div>
                                       )}
                                       {selectedCandidate.aiAnalysis && selectedCandidate.aiAnalysis.summary && (
                                          <div className="mt-4 pt-4 border-t border-slate-100">
                                              <div className="font-bold text-[10px] uppercase tracking-wider text-indigo-500 mb-2">AI Summary</div>
                                              <div className="bg-indigo-50/50 p-3 rounded-lg border border-indigo-100/50 text-slate-700 text-xs whitespace-pre-wrap">
                                                 {selectedCandidate.aiAnalysis.summary}
                                              </div>
                                          </div>
                                       )}
                                       {selectedCandidate.aiAnalysis && selectedCandidate.aiAnalysis.missingSkills && selectedCandidate.aiAnalysis.missingSkills.length > 0 && (
                                          <div className="mt-4 pt-4 border-t border-slate-100">
                                              <div className="font-bold text-[10px] uppercase tracking-wider text-rose-500 mb-2">Missing Skills</div>
                                              <div className="flex flex-wrap gap-1.5">
                                                 {selectedCandidate.aiAnalysis.missingSkills.map((s: string) => (
                                                    <Badge key={s} variant="outline" className="text-[10px] bg-rose-50 text-rose-700 border-rose-200">{s}</Badge>
                                                 ))}
                                              </div>
                                          </div>
                                       )}
                     ...
                     <div className="space-y-6">
                        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                            ...
                            <div className="space-y-3 text-sm">
                               ...
                               <p className="text-slate-600 flex justify-between"><span className="font-medium text-slate-900">Vendor:</span> <span className="truncate max-w-[150px]">{vendorMap[selectedCandidate.vendorId] || selectedCandidate.vendorName || selectedCandidate.vendorId || "Direct"}</span></p>
                            </div>
                        </div>
                        ...
                        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm mt-6">
                            ...
                            <div className="space-y-4">
                               <div className="relative pl-4 border-l-2 border-slate-100">
                                  ...
                                  <p className="text-xs font-medium text-slate-900">Candidate Created</p>
                                  <p className="text-[10px] text-slate-500 mt-0.5">{selectedCandidate.createdAt?.toDate ? selectedCandidate.createdAt.toDate().toLocaleString() : "Unknown"}</p>
                               </div>
                               {candidateEvents.map(evt => (
                                 <div key={evt.id} className="relative pl-4 border-l-2 border-indigo-100">
                                    <div className="absolute w-2 h-2 bg-indigo-500 rounded-full -left-[5px] top-1.5 ring-2 ring-white" />
                                    <p className="text-xs font-medium text-slate-900">{evt.type}</p>
                                    <p className="text-[10px] text-slate-500 mt-0.5">{evt.timestamp?.toDate ? evt.timestamp.toDate().toLocaleString() : "Recently"}</p>
                                 </div>
                               ))}
                            </div>
                        </div>
