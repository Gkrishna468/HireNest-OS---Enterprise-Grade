...
import { TrendingUp, Users, Target, Activity, Search, Award, BarChart3, Clock, ArrowUpRight, CheckCircle2, Shield, AlertTriangle } from "lucide-react";
...
export default function VendorIntelligenceTab() {
  ...
  useEffect(() => {
    const loadVendorIntelligence = async () => {
      ...
      setError(null);
      try {
        const data = await EnterpriseViewModelService.getVendorIntelligence();
        // Dynamic integrity scan
        ProductionDataGuard.validate(data, "Vendor Intelligence Leaderboard", "Enterprise View Model");
        setPerformances(data);
      } catch (err: any) {
        console.error("[Vendor Intelligence] Load failure:", err);
        setError(err?.message || "Failed to compile vendor metrics");
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="flex-1 overflow-y-auto p-8">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 text-slate-400">
            <Activity className="w-8 h-8 animate-spin text-indigo-600 mb-3" />
            <p className="text-sm font-medium">Aggregating live telemetry...</p>
            <p className="text-xs text-slate-400 mt-1">Reconstructing vendor scorecard indexes.</p>
          </div>
        ) : error ? (
          <div className="bg-rose-50 border border-rose-200 p-5 rounded-xl flex items-start gap-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-rose-800">Telemetry Compilation Alert</h4>
          ...
          <>
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
              ...
              <div className="bg-white p-5 border rounded-sm">
                <div className="flex items-center justify-between mb-4">
                  <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Avg Success Ratio</div>
                  <Target className="w-4 h-4 text-indigo-500" />
                </div>
                <div className="text-2xl font-black text-slate-800">
                  {performances.length ? Math.round(performances.reduce((acc, p) => acc + (p.successRate || 0), 0) / performances.length) : 0}%
                </div>
                <div className="text-sm text-slate-500 mt-1 flex items-center">
                  <ArrowUpRight className="w-3 h-3 text-emerald-500 mr-1" /> Core SLA compliant
                </div>
              </div>
              <div className="bg-white p-5 border rounded-sm">
                <div className="flex items-center justify-between mb-4">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100">
                      ...
                      <th className="py-3 px-6 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Avg Response</th>
                      ...
                      filteredPerformances.map((perf) => (
                        <tr key={perf.id} className="hover:bg-slate-50 transition-colors">
                          <td className="py-3 px-6">
                            <div className="flex items-center">
                              <div className="w-8 h-8 rounded bg-indigo-50 border border-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-xs mr-3">
                                {perf.name?.substring(0, 2).toUpperCase() || "VN"}
                              </div>
                              <div>
                                <div className="text-sm font-bold text-slate-800">{perf.name || "Unnamed Vendor"}</div>
                                <div className="text-xs text-slate-500">ID: {perf.id?.substring(0, 8)}...</div>
                              </div>
                            </div>
                          </td>
                          <td className="py-3 px-6">
                          ...
                          <td className="py-3 px-6">
                            <div className="flex items-center">
                              <div className="w-16 h-1.5 bg-slate-100 rounded-full mr-2 overflow-hidden">
                                <div 
                                  className={`h-full ${perf.placements > 0 ? 'bg-emerald-500' : 'bg-slate-400'}`} 
                                  style={{ width: `${perf.fillRate}` }}
                                />
                              </div>
                              <span className="text-xs font-medium text-slate-600">{perf.fillRate}</span>
                            </div>
                          </td>
                          ...
                          <td className="py-3 px-6 text-sm font-bold text-slate-800 text-right">
                            ₹{(perf.revenue || 0).toLocaleString("en-IN")}
                          </td>
                        </tr>
                      ))
                    )}
