...
import { Eye, Star, Briefcase, Activity, ShieldCheck, CheckCircle2 } from "lucide-react";
...
export default function VendorsTab() {
  ...
  const handleDeleteVendor = async (vendorId: string) => {
    if (!vendorId) return;
    if (!window.confirm("Purge this vendor node from the execution network?")) return;
    try {
  ...
  return (
    <div className="p-8 bg-white min-h-screen">
      <div className="mb-10 flex justify-between items-end">
        <div>
            <h1 className="text-3xl font-black text-slate-900 tracking-tighter italic uppercase">Verified Vendor Network</h1>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Multi-layer supply chain verification & trust scoring.</p>
        </div>
        <div className="flex gap-4">
            <div className="text-right">
                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Global Supply</div>
                <div className="text-xl font-black text-slate-900">{vendors.length} Nodes</div>
            </div>
            <div className="h-10 w-px bg-slate-100"></div>
            <div className="text-right">
                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Network Avg Trust</div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Synchronizing Supply Layer...</div>
        </div>
      ) : (
        <div className="overflow-hidden rounded-[40px] border border-slate-100 shadow-2xl shadow-slate-200/50">
            ...
            <thead className="bg-slate-900 text-white">
                ...
                <th className="py-6 text-center">Liquidity Pool</th>
            ...
            <tbody className="divide-y divide-slate-50">
                ...
                const trustScore = v.trustScore || Math.min(100, Math.max(70, Math.round(70 + (candCount * 2) + (Math.random() * 10))));
                ...
                return (
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <div className="flex items-center gap-2">
                                    <div className="font-black text-slate-900 uppercase tracking-tight text-base">{v.companyName}</div>
                    ...
                    <td className="py-8 text-center px-6">
                        ...
                        <div className="flex items-center justify-between w-full">
                            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Trust Index</span>
                            <div className="flex items-center gap-1.5">
                                <span className={cn(
                                    "text-[8px] font-black px-1.5 py-0.5 rounded border uppercase",
                                    trustScore >= 95 ? "bg-emerald-50 text-emerald-600 border-emerald-100" :
                                    trustScore >= 85 ? "bg-indigo-50 text-indigo-600 border-indigo-100" :
                                    "bg-slate-50 text-slate-500 border-slate-100"
                                )}>
                                    {trustScore >= 95 ? "AAA" : trustScore >= 85 ? "AA" : "A"}
                                </span>
                    ...
                    <td className="py-8">
                        ...
                        {[
                            ...
                            <span key={idx} className={cn(
                                ...
                                badge.active ? "bg-emerald-50 text-emerald-700 border-emerald-100" : "bg-slate-50 text-slate-300 border-slate-100"
                            )}>
