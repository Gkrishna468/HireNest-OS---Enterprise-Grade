...
export default async function handler(req: any, res: any) {
  ...
  if (!adminDb)
    return res.status(503).json({
      success: false,
      error:
        "Firebase Service Account configuration is missing. Cannot perform rebuild in client fallback mode.",
    });
  ...
  try {
    ...
    if (role !== "adminHQ") {
      return res
        .status(403)
        .json({ error: "Only Admin HQ can perform a hard matrix rebuild." });
    }
      ...
      for (const reqObj of requirements) {
        ...
        const hasSubmission = submissions.some(
          (sub: any) =>
            ...
            (sub.candidateId === cand.id ||
              sub.candidateId === cand.candidateId),
        );
...
{"matchScore": 85, "summary": "Strong fit based on React and Node.js experience.", "strengths": ["skill 1"], "missingSkills": ["skill 2"], "breakdown": {"skillsScore": 90, "experienceScore": 80, "domainScore": 80, "locationScore": 100}}`;
    ...
    return res
      .status(200)
      .json({ success: true, deletedMatchesCount, matchUpdatesCount });
  } catch (e: any) {
