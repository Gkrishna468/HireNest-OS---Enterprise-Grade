...
const generateAIPayload = async (orgId: string, systemInstruction: string, prompt: string, options: any) => {
   console.log(`[AI GATEWAY intel] Mocking generation for ${orgId}`);
   if (options?.responseMimeType === 'application/json') {
       return JSON.stringify({
           analysis: "This candidate shows strong potential but lacks specific senior leadership experience requested.",
           match_score: 75,
           red_flags: ["Gap in employment (2022)"],
           strengths: ["Strong technical foundation", "Good communication signals"]
       });
   }
   return "Mock intelligent analysis generated by local stub. Candidate appears to be a good fit.";
};
