...
export default async function submissionsHandler(req: any, res: any) {
  ...
  const { submissionId, newStatus, rejectReason, gapAnalysis, resumeSuggestions } = req.body;
  if (!submissionId || !newStatus) {
    return res.status(400).json({ success: false, error: 'Missing submissionId or newStatus' });
  }
  ...
  try {
    ...
    if (newStatus.toUpperCase() === 'REJECTED') {
      updates.rejectReason = rejectReason || 'Qualifications did not meet expectations';
      
      // Default / Override AI Gap Analysis fields for Candidate Recovery Center!
      updates.gapAnalysis = {
        missingSkills: gapAnalysis?.missingSkills || ['Enterprise Frameworks', 'Cloud Orchestration'],
        missingKeywords: gapAnalysis?.missingKeywords || ['CI/CD', 'Docker', 'Kubernetes'],
        interviewWeaknesses: gapAnalysis?.interviewWeaknesses || ['Architecture Systems Design'],
        resumeSuggestions: resumeSuggestions || 'Focus on highlighting full-lifecycle backend ownership and production operations.',
    ...
    const newSla = await EnterpriseRuntimeKernel.sla.initiateSLA(submissionId, newStatus.toUpperCase());
