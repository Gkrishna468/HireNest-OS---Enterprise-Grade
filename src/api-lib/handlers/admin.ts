...
const dispatchWorkflowEvent = async (db: any, payload: any) => {
  ...
  try {
    ...
    console.log(
      `[EVENT BUS] Dispatched: ${payload.eventType || payload.type} to system_events`,
    );
  } catch (e) {
...
export default async function handler(req: any, res: any) {
  ...
  if (!action) {
    if (rawPath.includes("metrics")) {
      ...
      rawPath.includes("strategy/analyze") ||
      rawPath.includes("strategy-analyze")
    ) {
      ...
      rawPath.includes("client-matches") ||
      rawPath.includes("client-ai-matches")
    ) {
  ...
  try {
    ...
    if (action === "metrics") {
      ...
      if (!adminDb) {
        console.warn(
          "[ADMIN METRICS] adminDb is falsy, returning zeroed metrics",
        );
        return res.status(200).json({
          revenue: 0,
          spending: 0,
      ...
      try {
        ...
        const [requirementsSnap, candidatesSnap, submissionsSnap] =
          await Promise.all([
            adminDb
              .collection("requirements_public")
              .get()
              .catch((e: any) => {
                console.error("[METRICS] req err", e);
                return { docs: [], size: 0 };
              }),
            adminDb
              ...
              .get()
              .catch((e: any) => {
                console.error("[METRICS] pool err", e);
                return { docs: [], size: 0 };
              }),
            adminDb
              ...
              .get()
              .catch((e: any) => {
                console.error("[METRICS] sub err", e);
                return { docs: [], size: 0 };
              }),
          ]);
        ...
        const result = {
          ...
          avgMargin:
            platformProfitSum > 0 && clientBudgetSum > 0
              ? (platformProfitSum / clientBudgetSum) * 100
              : 0,
    ...
    if (action === "approve") {
      if (!adminDb || !adminAuth) {
        return res.status(503).json({
          error: "Administrative runtime unavailable",
          status: "degraded",
        });
      }
    ...
    if (action === "strategy-analyze") {
      return res.status(200).json({
        analysis:
          "Strategic Engine Connected. Awaiting full IAM permission bindings on the Vercel Node for active data synthesis.",
      });
    }
    ...
    if (action === "governance-data" || action === "governance") {
      const collections = [
        ...
        "billingLedgers",
        "tenantEconomics",
        "tenantInfrastructureMap",
        "distributedTraces",
        "arbitrationLocks",
        "cognitiveMemoryGraphs",
        "infrastructureMigrations",
        "governancePolicies",
        "consensusLeases",
        "infrastructureSimulations",
        "recursiveReasoningTrees",
        "immuneQuarantines",
        "federatedBroadcasts",
        "tenant_usage",
        "ai_usage_logs",
    ...
    if (action === "notify-approval") {
      if (!adminDb) {
        return res
          .status(200)
          .json({ ok: true, note: "Admin DB offline fallback" });
      }
    ...
    if (action === "approve-requirement") {
      try {
        ...
        const financials = await computeFinancials(adminDb, {
          actualBudget:
            Number(actualBudget || req.body.financials?.clientBudget) || 0,
        ...
        await adminDb
          ...
          .update({
            ...
            financials: {
              ...
              clientCurrency:
                req.body.financials?.clientCurrency || currency || "INR",
              ...
              marginConfig: {
                type: "POLICY_ENGINE",
                value: financials.marginRate,
                policy: financials.appliedPolicy,
              },
            },
        ...
        try {
          ...
          console.warn(
            "No corresponding queue document was found, bypassing non-blocking update:",
            queueErr,
          );
        }
    ...
    if (action === "notifications") {
      ...
      try {
        const snap = await adminDb
          ...
          .limit(20)
          .get();
        const notes = snap.docs.map((doc: any) => ({
          id: doc.id,
          ...doc.data(),
        }));
    ...
    if (action === "telemetry") {
      try {
        ...
        if (!Array.isArray(traces) || traces.length === 0) {
          return res
            .status(400)
            .json({ success: false, message: "Invalid trace array payload" });
        }
        console.log(`[OTLP SINK] Ingested ${traces.length} distributed spans.`);
        return res
          .status(200)
          .json({ success: true, processedCount: traces.length });
      } catch (err: any) {
    ...
    if (action === "rebuild-revenue-pipeline") {
      if (!adminDb)
        return res
          .status(503)
          .json({ error: "Administrative runtime unavailable" });
      try {
        ...
        for (const req of requirements) {
          const reqMatches = matches.filter(
            (m: any) => m.requirementId === req.id,
          );
          const reqSubs = subs.filter((s: any) => s.requirementId === req.id);
          const reqDeals = deals.filter((d: any) => d.requirementId === req.id);

          const interviews =
            reqSubs.filter((s: any) => s.status === "INTERVIEWING").length +
            reqDeals.filter(
              (d: any) =>
                d.currentStage === "Interview" || d.currentStage === "Offer",
            ).length;
          const placements =
            reqSubs.filter(
              (s: any) => s.status === "HIRED" || s.status === "SELECTED",
            ).length +
            reqDeals.filter(
              (d: any) => d.currentStage === "Hired" || d.status === "WON",
            ).length;

          const pipelineValue = req.financials?.clientBilling
            ? (req.financials.clientBilling *
          ...
          reqMatches.forEach(
            (m: any) => (expectedRevenue += m.expectedRevenue || 0),
          );
    ...
    if (action === "migration-backfill") {
      ...
      try {
              ...
              .set(
                {
                  ...
                  orgId: orgId,
                  companyName:
                    data.name || data.companyName || "Unnamed CRM Account",
              ...
              .set(
                {
                  ...
                  orgId: orgId,
                  companyName:
                    data.companyName || data.name || "Unnamed Client",
                  orgType: "CLIENT",
                  tenantId: data.tenantId || "TENANT-HQ",
                  sourceSystem: "OS",
                  domain: domain,
              ...
              .set(
                {
                  ...
                  orgId: orgId,
                  companyName:
                    data.companyName || data.name || "Unnamed Vendor",
                  orgType: "VENDOR",
                  tenantId: data.tenantId || "TENANT-HQ",
                  sourceSystem: "OS",
                  domain: domain,
        ...
        for (const doc of orgsSnap.docs) {
          ...
          if (!data.orgType)
            updatesObj.orgType =
              (data.type || "").toUpperCase() === "VENDOR"
                ? "VENDOR"
                : "CLIENT";
          if (!data.companyName)
            updatesObj.companyName = data.name || "Unnamed Organization";
          if (!data.domain && !processedOrgs.has(data.id || doc.id))
            missingDomains++;
        ...
        return res
          .status(200)
          .json({ success: true, processed: updates.length, audit: auditLog });
      } catch (err: any) {
