...
export default async function handler(req: any, res: any) {
  ...
  if (!adminDb)
    return res.status(503).json({
      success: false,
      error:
        "Firebase Service Account configuration is missing. Cannot perform cleanup in client fallback mode.",
    });
  ...
  try {
    ...
    if (role !== "adminHQ") {
      return res
        .status(403)
        .json({ error: "Only Admin HQ can perform cleanup." });
    }
    ...
    if (reqId) {
      ...
      return res.status(200).json({
        success: true,
        deletedMatchesCount,
        message: `Force purged ${deletedMatchesCount} matches for req: ${reqId}`,
      });
    }
