...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const reqSnapshot = await adminDb
    ...
    for (const doc of allMatchesSnapshot.docs) {
      ...
      const matchReqId =
        matchData.canonicalRequirementId ||
        matchData.requirementId ||
        matchData.reqId ||
        doc.id;
    ...
    console.log(
      `Summary: Total ai_matches: ${allMatchesSnapshot.size}, After requirement filter: ${reqFiltered}, After active filter: ${activeFiltered}, After submission filter: ${submissionFiltered}, Final returned: ${matches.length}`,
    );
