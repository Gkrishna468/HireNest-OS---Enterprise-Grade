...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // Fetch Requirement
    const reqRef = adminDb.collection('requirements_public').doc(submission.requirementId || submission.reqId);
    const reqSnap = await reqRef.get();
    
    if (!reqSnap.exists) {
      await subRef.update({ status: 'MATCH_REJECTED', governanceReason: 'Requirement not found' });
    ...
    const jdText = (requirement.title + ' ' + (requirement.description || '')).toLowerCase();
    
    // Attempt to get candidate resume text to compare
    const candRef = adminDb.collection('candidatePool').doc(submission.candidateId);
    ...
    const combinedCandText = (resumeText + ' ' + candSkills + ' ' + (submission.candidateName||'')).toLowerCase();

    // 1. Mandatory Skills Check
    // A simple heuristic: checking intersection of major tech words in title
    const keyTechWords = ["react", "node", "java", "c#", ".net", "azure", "aws", "gcp", "python", "devops", "accounting", "sap", "biztalk", "sql"];
    const jdTechWords = keyTechWords.filter(w => jdText.includes(w));
    
    if (jdTechWords.length > 0) {
        const candTechWords = jdTechWords.filter(w => combinedCandText.includes(w));
        // If candidate meets less than 50% of the key tech words explicitly mentioned in the JD
        if (candTechWords.length === 0 && requirement.title.toLowerCase().match(/account|finance|tax|vat/)) {
             // Let's hardcode the domain rejection for Accounting vs SWE
             if (!combinedCandText.match(/account|finance|tax|vat|audit|ledger|cpa/)) {
                 matchStatus = 'MATCH_REJECTED';
                 reason = 'Domain mismatch: Candidate lacks mandatory domain experience.';
             }
    ...
    if (jdText.includes('accounting') && combinedCandText.includes('software engineer') && !combinedCandText.includes('accounting')) {
        matchStatus = 'MATCH_REJECTED';
        reason = 'Domain mismatch';
    }

    // 2. Experience Check
    const reqExpYears = extractYearsOfExperience(jdText) || requirement.yearsOfExperience || 0;
    const candExpYears = extractYearsOfExperience(candExp) || extractYearsOfExperience(combinedCandText);
    
    if (reqExpYears > 0 && candExpYears > 0) {
        if (candExpYears < reqExpYears - 2) { // Give 2 years leeway
            matchStatus = 'MATCH_REJECTED';
