...
export class StateEngine {
  ...
  async transitionState(
    ...
    currentState: string,
    newState: string,
    context?: { role: string; submissionId: string },
  ): Promise<boolean> {
    ...
    if (context) {
      ...
      if (
        ...
        (normalizedNew === "REJECTED" ||
          (normalizedCurrent !== "REJECTED" && normalizedNew !== "SUBMITTED"))
      ) {
        console.warn(
          `[StateEngine] Rejected transition: Vendor attempted unauthorized transition to ${normalizedNew}`,
        );
        return false;
      }
    }
    ...
    if (
      ...
      normalizedCurrent !== "JOINED"
    ) {
      await this.logEvent(
        context?.submissionId || "unknown",
        normalizedCurrent,
        normalizedNew,
      );
      return true;
  ...
  private async logEvent(
    ...
    try {
      ...
      console.log(
        `[StateEngine] Immutable event logged: Submission ${submissionId} transitioned from ${fromState} to ${toState}`,
      );
    } catch (e) {
