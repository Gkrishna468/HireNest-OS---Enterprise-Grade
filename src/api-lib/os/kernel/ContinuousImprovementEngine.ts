...
export class ContinuousImprovementEngine {
    ...
    async executeImprovementLoop(orgId: string): Promise<void> {
        ...
        const observationsSnapshot = await db.collection(ContinuousImprovementEngine.OBSERVATIONS_COLL)
            .where('orgId', '==', orgId)
            .get();

        const observations = observationsSnapshot.docs.map(doc => doc.data() as Observation);
        ...
        observations.forEach(obs => {
            if (obs.eventType === 'REJECTION' && obs.fact) {
                ...
                clientRejectionReasons[clientId][obs.fact] = (clientRejectionReasons[clientId][obs.fact] || 0) + 1;
            }
        });
        ...
        for (const [clientId, facts] of Object.entries(clientRejectionReasons)) {
            ...
            for (const [fact, count] of Object.entries(facts)) {
                const calculatedWeight = parseFloat((count / totalRejections).toFixed(2));
                ...
                await db.collection(ContinuousImprovementEngine.EXPERIENCES_COLL).doc(expId).set(expData);
                console.log(`[Continuous Improvement] Synthesized experience: ${expId} with weight ${calculatedWeight}`);
            }
        }
     ...
     */
    async logObservation(orgId: string, office: string, targetId: string, eventType: string, fact: string, details: any = {}): Promise<void> {
        ...
        await db.collection(ContinuousImprovementEngine.OBSERVATIONS_COLL).doc(obsId).set(observation);
        console.log(`[Continuous Improvement] Logged raw observation: ${obsId} for ${targetId}`);

        // Trigger real-time incremental experience evolution
        await this.evolveExperienceIncrementally(orgId, office, targetId, eventType, fact);
    }

    /**
     * Backward-compatible updateExperience mapping observations and experiences
     */
    async updateExperience(orgId: string, office: string, observationPayload: any): Promise<void> {
        console.log(`[Continuous Improvement] Legacy updateExperience called for ${office} in ${orgId}`);
        ...
        await this.logObservation(orgId, office, targetId, eventType, fact, observationPayload);
    }

    private async evolveExperienceIncrementally(orgId: string, office: string, targetId: string, eventType: string, fact: string): Promise<void> {
        ...
        const expDoc = await db.collection(ContinuousImprovementEngine.EXPERIENCES_COLL).doc(expId).get();

        if (expDoc.exists) {
            const current = expDoc.data() as Experience;
            const newCount = current.observationCount + 1;
            const newWeight = parseFloat(Math.min(1.0, current.weight + 0.05).toFixed(2)); // strengthen conclusion weight
            
            await db.collection(ContinuousImprovementEngine.EXPERIENCES_COLL).doc(expId).update({
            ...
            await db.collection(ContinuousImprovementEngine.EXPERIENCES_COLL).doc(expId).set(initialExp);
        }
    }

    async getExperiencesForTarget(orgId: string, targetId: string): Promise<Experience[]> {
        const snapshot = await db.collection(ContinuousImprovementEngine.EXPERIENCES_COLL)
            .where('orgId', '==', orgId)
