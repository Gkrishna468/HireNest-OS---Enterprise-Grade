...
export class SLAEngine {
  ...
  async initiateSLA(submissionId: string, stage: string): Promise<SLADetails> {
    ...
    else if (stage.startsWith("INTERVIEW") || stage.includes("ROUND"))
      targetHours = 72;
    else if (stage === "OFFER") targetHours = 96;

    const startedAt = new Date();
    const deadline = new Date(
      startedAt.getTime() + targetHours * 60 * 60 * 1000,
    );
    ...
    try {
      ...
      console.error(
        `[SLAEngine] Error updating submission SLA for ${submissionId}:`,
        e,
      );
    }
  ...
  async completeSLA(submissionId: string, reason?: string): Promise<void> {
    try {
      ...
      await subRef.update({
        ...
        "sla.breachReason": breached
          ? reason || "Stage SLA limit exceeded"
          : undefined,
      });
    } catch (e) {
  ...
  async checkSLA(
    submissionId: string,
  ): Promise<{ status: "GREEN" | "YELLOW" | "RED"; timeRemaining?: number }> {
    try {
      const subRef = db.collection("submissions").doc(submissionId);
      const subSnap = await subRef.get();
